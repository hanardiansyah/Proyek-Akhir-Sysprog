...
#define  DEVICE_NAME "gipa"    // /dev/<value>
#define  CLASS_NAME  "gipa"          // The device class

MODULE_LICENSE("GPL");            
MODULE_AUTHOR("Hana Fadhila Ardiansyah & Nadia Victoria Aritonang");    
...
static int dev_open(struct inode *inodep, struct file *filep){
   numberOpens++;
   printk(KERN_INFO "gipadevdr: Device has been opened %d time(s)\n", numberOpens);
   return 0;
}

static int dev_release(struct inode *inodep, struct file *filep){
   printk(KERN_INFO "gipadevdr: Device successfully closed\n");
   return 0;
}
...
static int __init rdchardrv_init(void){
   ...
   printk(KERN_INFO "gipadevdr: device class created correctly\n");
   return 0;
}
...
static void __exit rdchardrv_exit(void){
    ...
    class_destroy(classptr);                              
    unregister_chrdev(majorNumber, DEVICE_NAME);              
    printk(KERN_INFO "gipadevdr: Goodbye from the LKM!\n");
}
